...
require 'utils.scaled_margin_cub'
optim_utils = require 'utils.adam_v2'

-- parse command-line options
opts = lapp[[
  --saveFreq      (default 40)        save every saveFreq epochs
...
opts.modelName = string.format('CUB_%s_adam%d_bs%d_zdim(%d,%d)_wd%g_alpha%g', 
...
if opts.modelString == '' then
  ...
  cubdiscvae_module = dofile('scripts/' .. opts.modelString .. '.lua')
  encoder_fg, encoder_bg, decoder_fg, decoder_bg = cubdiscvae_module.create(opts)
end
...
cub.setScale(opts.loadScale)
trainData = cub.loadTrainSet()
--mean, std = image_utils.normalizeGlobal(trainData.data)
trainData:scaleData()

valData = cub.loadValSet()
...
for t = epoch+1, opts.maxEpoch do
  ...
  for i = 1, N_train, opts.batchSize do 
    ...
    for j = i, i+opts.batchSize-1 do
      ...
      -- cropping
      local cpx = math.random(opts.loadScale - opts.scale + 1)
      local cpy = math.random(opts.loadScale - opts.scale + 1)
      cur_im = cur_im[{{}, {cpx, cpx + opts.scale - 1}, {cpy, cpy + opts.scale - 1}}]:clone()
      cur_mask = cur_mask[{{}, {cpx, cpx + opts.scale - 1}, {cpy, cpy + opts.scale - 1}}]:clone()
      -- rotation
      local randRot = torch.rand(1)*0.16-0.08
      cur_im = image.rotate(cur_im, randRot:float()[1], 'bilinear')
      cur_mask = image.rotate(cur_mask, randRot:float()[1], 'bilinear')
